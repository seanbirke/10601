...
class NN:
	...
	def forward(self, x):
		...
		# print("alpha:", self.W[0].shape, self.W[0])
		# return
...
def load_data(path):
	...
	for line in lines:
		...
		data.append(np.array(data_vec).reshape(1, num))
	return data, labels
...
def run(train_input, test_input, train_out, test_out, metrics_out, num_epoch, hidden_units, init_flag, lr):
	...
	model = NN(input_size = num, hidden_size = hidden_units, output_size = 10, lr = lr, init_flag = init_flag)
